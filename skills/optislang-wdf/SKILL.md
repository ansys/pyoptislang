...
See [references/osl_components.md](./references/osl_components.md) for the full element taxonomy with all `plugInId` and `source` URN values and registered locations details.
...
```json
{
    ...
    "root": {
        ...
        "properties": { /* see references/osl_components.md #RunnableSystem properties */ },
        "children": {
            "workflow": {
                "type": "DataDependency",
                "connections": [],
...
- `datapins` and `inner_datapins` are **only added when the user explicitly requests them**. The full pin sets are documented in [references/osl_components.md](./references/osl_components.md) for reference.
...
4. **Build the tree from outside in:**
    - Start with `root` as `ControlStatementPlugIn` / `plugInId: "RunnableSystem"` with properties (see [references/osl_components.md](./references/osl_components.md)). Do **not** add `datapins` or `inner_datapins` unless the user explicitly asks for them.
...
- **`datapins` and `inner_datapins` are omitted by default** — only add them when the user explicitly requests it. Full pin sets are in [references/osl_components.md](./references/osl_components.md).
