...
def _get_system_vars(pattern=".*"):
    ...
    sys_vars = os.environ
    varnames = list(sys_vars._data.items())
    if os.name == "posix":
        varnames = [[var.decode("utf-8") for var in sets] for sets in varnames]
    dictionary = {}
    for varname in varnames:
        varname_match = re.search(pattern, varname[0])
        if varname_match:
            dictionary[varname[0]] = varname[1]
    return dictionary
